...
MultimodalRAG is a high-accuracy document parsing engine specifically optimized for Multimodal RAG (Retrieval-Augmented Generation) and LLM Agent workflows. It converts PDF, DOCX, PPTX, XLSX, Images, and Web pages into structured Markdown and JSON formats, enabling seamless downstream QA, retrieval, and analysis.

## Key Features

- **Multi-Format Input**: Support for PDF, images, DOCX, PPTX, and XLSX files.
- **RAG-Optimized**: Automatic removal of layout noise (headers, footers, footnotes, page numbers) to maintain semantic coherence.
- **Formulas & Tables**: Converts formulas to LaTeX and tables to HTML for high-fidelity extraction.
- **OCR Support**: Built-in VLM + OCR dual engines supporting 109 languages for scanned and handwritten documents.
- **Flexible Deployment**: Supports CLI, FastAPI, and Gradio WebUI. Runs on CPU, GPU, and various hardware platforms.
...
MultimodalRAG is a document parsing tool that converts `PDF`, image, `DOCX`, `PPTX`, and `XLSX` inputs into machine-readable formats such as Markdown and JSON for downstream retrieval, extraction, and processing.
MultimodalRAG was born during the pre-training process of [InternLM](https://github.com/InternLM/InternLM). We focus on solving symbol conversion issues in scientific literature and hope to contribute to technological development in the era of large models.
Compared to well-known commercial products, MultimodalRAG is still young. If you encounter any issues or if the results are not as expected, please submit an issue on [issue](https://github.com/opendatalab/MinerU/issues) and **attach the relevant document or sample file**.
