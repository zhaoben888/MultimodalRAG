...
MultimodalRAG 是一款专为**多模态 RAG（检索增强生成）**及 LLM Agent 场景构建的高精度文档解析引擎。它可以将 PDF、DOCX、PPTX、XLSX、图片及网页转化为结构化的 Markdown 和 JSON 格式，为后续的检索、问答和知识库构建提供高质量的数据支撑。

## 主要功能

- **多格式输入支持**：支持 PDF、图片以及 DOCX、PPTX、XLSX 等常见办公文档。
- **专为 RAG 优化**：自动删除页眉、页脚、页码和非语义噪音，确保文本块语义连贯。
- **公式与表格还原**：将公式完美转化为 LaTeX 语法，表格转化为 HTML 代码，精准保存结构化信息。
- **强大 OCR 支持**：VLM 与 OCR 双引擎驱动，支持 109 种语言，完美处理扫描件与手写内容。
- **简便部署**：内置命令行、FastAPI 接口和 Gradio WebUI，支持 CPU/GPU 及多种国产算力平台。
...
MultimodalRAG 是一款文档解析工具，可将 `PDF`、图片以及 `DOCX`、`PPTX`、`XLSX` 转化为机器可读格式（如 Markdown、JSON），便于后续检索、抽取与二次处理。
MultimodalRAG诞生于[书生-浦语](https://github.com/InternLM/InternLM)的预训练过程中，我们将会集中精力解决科技文献中的符号转化问题，希望在大模型时代为科技发展做出贡献。
相比国内外知名商用产品MultimodalRAG还很年轻，如果遇到问题或者结果不及预期请到[issue](https://github.com/opendatalab/MinerU/issues)提交问题，同时**附上相关文档或样例文件**。
...
## 🛠️ Web RAG 智能图文问答助手 (本地 Demo)

本仓库在 MinerU 解析引擎的基础上，集成开发了一套本地**多模态图文问答助手**应用。包含基于 FastAPI 的后端服务与基于极简毛玻璃特效的现代暗黑风前端界面，支持文本/表格/插图提取、语义+关键词混合检索、大模型流式图文问答及本地知识库异步扫描挂载。

### 1. 快速启动
在确保您的本地开发环境已成功安装 MinerU 的全部依赖后，在子目录 `MultimodalRAG` 内运行以下命令启动服务：
```bash
...
### 2. 初始化配置
首次使用时，请点击页面右上角的 **“配置 API Key”**：
1. **阿里百炼 API Key**：用于驱动 `text-embedding-v3` 计算切片的语义向量 Embedding。
2. **DeepSeek API Key**：用于调用大语言模型 `deepseek-chat` 以生成多模态参考流式回答。

### 3. 系统特色
- 📌 **多模态图文对齐**：通过 JIT 图片路径纠偏机制，将 MinerU 提取的公式/图表在回答中以流式 Markdown 和真实图片形式渲染输出。
- 🔍 **Hybrid Score 混合检索**：基于余弦相似度语义匹配，并融合中英文与数字型号关键词的 Boost 加权得分，大幅提高特定国标、产品序列号检索召回率。
- 📂 **本地异步扫描挂载**：在页面中直接输入本地磁盘目录，通过 FastAPI 后台异步线程自动扫描挂载新文档并持久化到内存向量数据库。
